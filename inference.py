...
def build_restormer(num_blocks=[4, 6, 6, 8], num_heads=[1, 2, 4, 8], channels=[48, 96, 192, 384],
    ...
    """
    Build the Restormer model.

    Args:
        num_blocks (list): Number of transformer blocks at each stage.
        num_heads (list): Number of attention heads at each stage.
        channels (list): Number of channels at each stage.
        num_refinement (int): Number of refinement blocks.
        expansion_factor (float): Expansion factor for the feed-forward network.

    Returns:
        Restormer: The Restormer model.
    """
    model = Restormer(num_blocks=num_blocks, num_heads=num_heads, channels=channels,
                      num_refinement=num_refinement, expansion_factor=expansion_factor)
    model.load_state_dict(torch.load("models/restormer_model.pth", map_location=DEVICE))
